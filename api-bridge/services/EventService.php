...
"SELECT * FROM event WHERE start_time <= ? AND end_time>= ? ORDER BY start_time ASC",
    ...
    */
    ...
    if ($seconds < 0) return '0 dakika' ; $days=floor($seconds / 86400); $hours=floor(($seconds % 86400) / 3600);
        $minutes=floor(($seconds % 3600) / 60); $parts=[]; if ($days> 0) $parts[] = "{$days} gün";
